...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    // 1. Seek to the correct page position
    lseek(fd, page_no * PAGE_SIZE, SEEK_SET);//定位到文件头，通过(fd,page_no)可以定位指定页面及其在磁盘文件中的偏移量
    
    // 2. Write the data and check return value
    ssize_t bytes_written = write(fd, offset, num_bytes);//调用write()函数
 ...
 */
page_id_t DiskManager::allocate_page(int fd) {//分配一个新的页号
    // 简单的自增分配策略，指定文件的页面编号加1
    assert(fd >= 0 && fd < MAX_FD);
    return fd2pageno_[fd]++;
}

void DiskManager::deallocate_page(__attribute__((unused)) page_id_t page_id) {}//释放一个页号

bool DiskManager::is_dir(const std::string& path) {//判断指定路径是否是一个目录
    struct stat st;
    return stat(path.c_str(), &st) == 0 && S_ISDIR(st.st_mode);
}

void DiskManager::create_dir(const std::string &path) {//创建一个目录
...
void DiskManager::destroy_dir(const std::string &path) {//删除一个目录
 ...
 */
void DiskManager::destroy_file(const std::string &path) {//删除指定路径的文件
    // Todo:
    // 调用unlink()函数
    // 注意不能删除未关闭的文件
    if (!is_file(path)) {
